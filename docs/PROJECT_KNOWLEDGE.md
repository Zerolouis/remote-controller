...
当前 C++ 后端接口位于 `packages/remote_controller_core/native/include/backends/`，协议布局位于 `native/include/controller_protocol.h`。`Session` 组合具体后端并拥有状态机、sequence 校验和 watchdog；链路诊断组合为异步 `LoopbackTransportBackend` + `MemoryVirtualControllerBackend`。`SdlInputBackend` 是首个实体 `InputBackend`，`VigemVirtualControllerBackend` 是首个系统虚拟手柄实现。`InputCapture` 提供独立的设备验证生命周期，`LocalControllerBridge` 则在单个原生对象内组合 SDL → ViGEm → SDL 震动，用于在网络和 HidHide 接入前验证完整硬件链路；两者不会同时打开同一设备。
...
当前公开状态结构均以 `struct_size` 开头：`rc_session_snapshot_v1` 为 56 字节，`rc_sdl_runtime_info_v1` 为 336 字节，`rc_input_device_info_v1` 为 712 字节，`rc_input_capture_snapshot_v1` 为 64 字节，`rc_vigem_runtime_info_v1` 为 272 字节，`rc_vigem_installer_launch_result_v1` 为 16 字节，`rc_local_bridge_snapshot_v1` 为 56 字节；`rc_gamepad_state_v1` 与 wire 明文同为 16 字节。opaque handle 不跨 DLL 暴露 C++、STL 或线程对象。Dart `LoopbackSession`、`SdlInputCapture` 和 `LocalControllerBridge` 都是显式 `close()` 资源；Flutter 只以 10 Hz 读取诊断快照，250 Hz 采样、ViGEm 提交和震动执行不经 Dart 往返。
...
- Flutter Windows 工程位于 `apps/remote_controller`，已有 Client/Server 角色选择、原生 ABI 健康状态、本机链路安全自检、SDL 设备列表、原始状态记录、ViGEmBus 状态和本机 SDL → ViGEm 桥接界面。
- 核心包位于 `packages/remote_controller_core`，Native Assets 编译 C++20 Session、SDL input、input capture、loopback transport、memory virtual controller、ViGEm X360 backend、本机桥接和 C ABI，并把 `SDL3.dll` 打包到应用旁；ViGEmClient 源码静态编入核心 DLL。
...
- 尚无 HidHide、ENet 或密码学实现；UI 的真实局域网会话启动按钮继续禁用。本机桥接会创建系统虚拟手柄，但不会隐藏实体手柄，可能产生双输入。
- Core 12 项和 Flutter 应用 11 项测试已通过；覆盖 ViGEmBus probe、target 创建成功后输入打开失败清理、缺失安装器路径拒绝、原生固定哈希拒绝、Dart 安装器哈希失败、缓存复用、UAC 取消和安装后重新探测 UI。
- Windows x64 Release 已构建。发布目录包含 `remote_controller.exe`、`remote_controller_core.dll`、`SDL3.dll` 和 Flutter 运行库；不包含 ViGEmBus 安装器或 `ViGEmClient.dll`。`remote_controller_core.dll` 只依赖系统 `SETUPAPI.dll`、`SHELL32.dll`、`bcrypt.dll`、`KERNEL32.dll`。开发机 `ViGEmBus` 服务为 Running；未安装驱动机器上的完整 UAC 安装流程、掌机实际桥接和震动仍待实测。
...
4. 把本机桥接拆分为未来 Client/Server 两侧可复用的 session 组合：Client SDL → transport，Server transport → ViGEm，并复用现有 sequence/watchdog 安全底座。
5. 实现 M2 最小局域网通道和能力握手，保持控制/震动可靠通道与低延迟完整状态通道分离。
6. 设计并实现最小特权 HidHide broker；在实机独占验证前保持默认不自动修改驱动配置。
7. 向 `native_toolchain_c` 上游报告中文 `vswhere -utf8` 解码和 C++ `/TP` 链接问题，升级后移除兼容代码。
