...
最后更新：2026-07-23
状态：M0 初始化完成；M1 SDL 实体输入枚举与原始值捕获完成，ViGEm 输出待实现
许可证：GPL-3.0-only
...
当前 C++ 后端接口位于 `packages/remote_controller_core/native/include/backends/`，协议布局位于 `native/include/controller_protocol.h`。`Session` 组合具体后端并拥有状态机、sequence 校验和 watchdog；链路诊断组合为异步 `LoopbackTransportBackend` + `MemoryVirtualControllerBackend`。`SdlInputBackend` 已作为首个实体 `InputBackend` 实现，`InputCapture` 在网络会话接入前提供独立的设备验证生命周期；后续 ENet/ViGEm 不改变上层会话 ABI。
...
当前公开状态结构均以 `struct_size` 开头：`rc_session_snapshot_v1` 为 56 字节，`rc_sdl_runtime_info_v1` 为 336 字节，`rc_input_device_info_v1` 为 712 字节，`rc_input_capture_snapshot_v1` 为 64 字节；`rc_gamepad_state_v1` 与 wire 明文同为 16 字节。opaque handle 不跨 DLL 暴露 C++、STL 或线程对象。Dart `LoopbackSession` 和 `SdlInputCapture` 都是显式 `close()` 资源；Flutter 只以 10 Hz 读取捕获快照，250 Hz 实时采样不经 Dart 往返。
...
- Flutter Windows 工程位于 `apps/remote_controller`，已有 Client/Server 角色选择、原生 ABI 健康状态、本机链路安全自检、SDL 设备列表和原始状态记录界面。
- 核心包位于 `packages/remote_controller_core`，Native Assets 编译 C++20 Session、SDL input、input capture、loopback transport、memory virtual controller 和 C ABI bridge，并把 `SDL3.dll` 打包到应用旁。
- `rc_get_*`、`rc_sdl_*`、`rc_input_capture_*` 与 `rc_session_*` 全部通过 `tool/ffigen.dart` 生成的 `@Native` 绑定接入 Dart，没有手写 FFI 函数或结构。
...
- SDL `3.4.12` 已固定并精确校验；可枚举 VID/PID、GUID、路径、类型、连接状态、支持按钮、模拟扳机和震动能力。ROG Ally X 当前只按 `0x0B05/0x1B4C` 标记候选，尚未完成实机拓扑矩阵。
- `InputCapture` 在原生线程每 4 ms 读取完整状态，不做死区、曲线或平滑；停止/断开立即把快照状态清零。Flutter 每 100 ms 读取一次快照，不参与采样。
- 尚无 HidHide、ENet、密码学或 ViGEm 实现；UI 的真实会话启动按钮继续禁用，原始值记录不会修改 HidHide，也不会创建系统虚拟手柄。
- Core 9 项和 Flutter 应用 5 项测试已通过；Windows Release 已构建并确认同时包含 `remote_controller_core.dll` 与 `SDL3.dll`。链接阶段继续使用独立 `CLinker` 规避 `native_toolchain_c 0.19.2` 的 MSVC `/TP` 问题。

紧接任务：

1. 在 ROG Ally X 实机运行原始值记录，保存不同 Armoury Crate 控制模式下的 SDL 名称、GUID、VID/PID、路径、支持按钮和轴/扳机范围。
2. 固定 ViGEmClient 依赖，实现驱动检测、单 X360 target、完整状态映射和震动回调；复用现有 Session watchdog。
3. 把组合测试扩展为 SDL → Session → memory virtual，再替换为 ViGEm 进行实体输入到虚拟输出验证。
4. 为 `InputBackend` 增加震动入口，把 ViGEm 回调通过原生队列回送 SDL；回调和网络路径均不得进入 Flutter UI 线程。
5. 设计并实现最小特权 HidHide broker；在实机验证前保持默认不自动修改驱动配置。
