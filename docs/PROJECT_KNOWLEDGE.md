...
最后更新：2026-07-23
状态：M0 初始化完成；M1 SDL 输入、ViGEm X360 输出、本机震动桥接和 ViGEmBus 安装入口完成，待掌机实测
许可证：GPL-3.0-only
...
- HidHide 只做检测和官方安装说明。ViGEmBus 不随应用捆绑，仅在用户明确点击后从官方固定 URL 下载、校验 SHA-256，并通过标准 Windows UAC 启动交互式安装器。
...
| 6 | 2 | flags | 正式 AEAD 为 0；`0x0001` 仅表示当前可信 LAN plaintext 诊断帧 |
...
当前 M2 首个可运行增量复用同一 64 字节布局，但将 16 字节状态直接放入 `encrypted_state` 区、认证标签固定全零，并必须设置 `RC_INPUT_FLAG_DIAGNOSTIC_PLAINTEXT (0x0001)`。`UdpLanTransportBackend` 只在用户明确启动的“未加密诊断版”中接受该标志，并同时校验随机 session ID、TCP 对端 IPv4、严格递增 sequence、长度和保留字段。正式配对 transport 不得接受此标志；接入 AEAD 后 diagnostic plaintext 默认关闭。
...
当前 diagnostic control 使用固定 32 字节小端帧，magic 为 `RCC1`，消息仅实现 `HELLO`、`HELLO_ACK`、`HEARTBEAT`、`RUMBLE`、`STOP` 和错误断开；它没有身份、能力协商、加密或重放窗口，不属于最终安全握手。Client 每 1 秒 heartbeat，Server 3 秒未收到即立即断开并归零。震动只由 Server TCP control worker 发送，ViGEm callback 不执行 socket I/O。
...
`VigemVirtualControllerBackend` 已实现单 target 生命周期、完整 XUSB 映射和通知回调。16 位扳机仅在 `ToXusbReport()` 最终边界以 `(value * 255 + 32767) / 65535` 做最近整数量化；四轴直接写入 XUSB `SHORT`，不施加死区或曲线。ViGEm 低频/高频 8 位马达值分别乘 `257` 恢复到 SDL 使用的 16 位域。本机诊断 `LocalControllerBridge` 不启用 HidHide，因此实体和虚拟手柄会同时暴露给其他进程，UI 必须持续显示双输入警告。
...
- 连续记录超过 25,000 个样本，约 250 Hz，按键和轴输入正常；该结果只验证 SDL 采样，ViGEm 本机桥接和震动仍需在掌机上完成下一轮实测。
...
决定：ViGEmBus/HidHide 均不随应用安装包分发。ViGEmBus 允许在用户明确操作后由应用下载官方固定资产、做大小与 SHA-256 校验，再启动原始交互式安装器；HidHide 仍只提供检测和官方来源。这样保留便捷安装，同时避免重新打包签名驱动或静默改变系统。
...
决定：M2 先实现明确标记的 trusted-LAN diagnostic transport，使用同端口的独立 TCP/UDP socket 验证 Client SDL → UDP → Server ViGEm 和 TCP 震动回程。它复用最终 64 字节输入布局、随机 session ID、严格 sequence 和 watchdog，但不声称安全，也不自动发现或持久化对端。UI 必须持续显示“未加密诊断版”；正式 libsodium/DPAPI 配对实现后，production transport 拒绝 plaintext 标志。这样可先在两台 Windows 设备上定位防火墙、Wi-Fi、线程和驱动问题，而不把临时协议误当成最终协议。
...
| 仅凭 `0x0B05/0x1B4C` 标识 ROG Ally X | 固件或控制模式变化时可能漏识别；同 VID/PID 设备也不能直接作为 HidHide 唯一键 | UI 将其标为候选，独占阶段仍用 Windows 设备实例路径和容器 ID 二次确认 |
...
| 实体手柄和 ViGEm target 在 HidHide 前同时可见 | 本机桥接诊断可能在游戏中产生双输入 | UI 持续警告；M1 只用于验证，不声明独占；M3 接入 HidHide 后再作为正式会话 |
...
| 当前 LAN diagnostic 未认证或加密 | 同一网络内攻击者可观察或注入控制器状态 | UI 明示只用于可信 LAN；限制 session ID/TCP 对端 IP；下一步接入身份配对和 AEAD，生产模式拒绝 plaintext 标志 |
| 当前 LAN diagnostic 只支持 IPv4 单客户端 | IPv6-only、复杂 DNS 或多客户端环境不可用 | MVP 实机先验证 IPv4；发现和地址族扩展在安全握手后处理 |
...
- [x] 首个可信 LAN 诊断通道：TCP control + UDP 完整状态、随机 session ID、对端 IPv4 限制、严格 sequence、100 ms watchdog、震动回传和统计 UI。
- UDP discovery、正式 TCP control framing 和 ENet/UDP input backend。
...
- `rc_get_*`、`rc_sdl_*`、`rc_input_capture_*`、`rc_vigem_*`、`rc_local_bridge_*` 与 `rc_session_*` 全部通过 `tool/ffigen.dart` 生成的 `@Native` 绑定接入 Dart，没有手写 FFI 函数或结构。
...
- 已有真实 Windows TCP/UDP socket 链路，但尚无 discovery、ENet、能力协商、身份、AEAD、DPAPI 或 HidHide。UI 明确标注网络功能为“未加密诊断版”；本机程序仍可能收到实体手柄，可信 LAN 之外不得使用。
- Core 14 项和 Flutter 应用 13 项测试已通过；新增测试在 localhost 通过 TCP HELLO 建立会话，发送带原始极值的 64 字节 UDP 完整状态，验证 ViGEm 提交和 watchdog 中立；检测到实体 SDL 手柄时还会条件式验证原生 Client→Server 持续发送。真实双机 Wi-Fi、防火墙、断线和震动仍需掌机/PC 实测。
- Windows x64 Release 已构建。发布目录包含 `remote_controller.exe`、`remote_controller_core.dll`、`SDL3.dll` 和 Flutter 运行库；不包含 ViGEmBus 安装器或 `ViGEmClient.dll`。网络实现只新增系统 `WS2_32.dll`，其余核心系统依赖为 `SETUPAPI.dll`、`SHELL32.dll`、`bcrypt.dll` 和 `KERNEL32.dll`。ROG Ally X 已验证本机 SDL → ViGEm 按键映射和持续采样；未安装驱动机器上的完整 UAC 流程、双机 LAN 和端到端震动仍待实测。

紧接任务：

1. 在未安装 ViGEmBus 的测试机上验证应用内下载、UAC、安装完成/取消、需要重启和重新检测路径；确认 Release 目录始终不包含驱动安装器。
2. 在 ROG Ally X 上启动“桥接到虚拟 X360”，用 Windows 游戏控制器面板或测试游戏验证按钮/轴/扳机、target 销毁和双马达震动回传；当前不要在要求独占输入的游戏里长时间运行。
3. 补充不同 Armoury Crate 控制模式下的 SDL 名称、GUID、路径和 Windows 设备拓扑矩阵，为 HidHide 精确目标选择提供依据。
4. 在 ROG Ally X 与 PC 两台机器上验证 diagnostic LAN：防火墙提示、250 Hz UDP、按钮边沿、原始轴/扳机、100 ms 拔网线归零、正常 Stop、睡眠唤醒和双向震动。
5. 在现有分离通道上加入 capability HELLO、UDP discovery 和可展示的网络延迟/抖动统计；保持 plaintext 标志只用于诊断。
6. 固定并审计 libsodium 版本，接入 Ed25519/X25519/HKDF/ChaCha20-Poly1305、DPAPI 身份存储和六位确认，随后默认禁用未加密 transport。
7. 设计并实现最小特权 HidHide broker；在实机独占验证前保持默认不自动修改驱动配置。
8. 向 `native_toolchain_c` 上游报告中文 `vswhere -utf8` 解码和 C++ `/TP` 链接问题，升级后移除兼容代码。
