...
当前 C++ 接口骨架位于 `packages/remote_controller_core/native/include/backends/`，协议布局位于 `native/include/controller_protocol.h`。
...
- loopback TransportBackend：同机 Client→Server 验证按钮、轴、扳机与中立超时。
- 原生 session/state/event C ABI 和 FFI 状态页。
...
- Flutter Windows 工程位于 `apps/remote_controller`，已有 Client/Server 角色选择、原生 ABI 健康状态和两种占位流程。
- 核心包位于 `packages/remote_controller_core`，Native Assets 编译 `native/src/remote_controller_core.cpp`。
- `rc_get_abi_version` 与 `rc_get_build_info` 已通过 FFIgen 生成的 `@Native` 绑定接入 Dart。
...
- 尚无 SDL、HidHide、ENet、密码学、ViGEm 或真实会话实现；UI 启动按钮明确禁用。
- `flutter test` 已验证 Debug Native Assets 和 FFI 调用；`flutter build windows` 已生成 Release 可执行文件。链接阶段使用独立 `CLinker` 规避 `native_toolchain_c 0.19.2` 的 MSVC `/TP` 问题。

紧接任务：

1. 增加 opaque session ABI、状态机和内存 loopback 后端。
2. 引入 SDL 3 固定版本并完成 ROG Ally X 实机枚举/原始值记录工具。
3. 引入固定 ViGEmClient 依赖并实现单 X360 target、震动回调和 100 ms 中立 watchdog。
