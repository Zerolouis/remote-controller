...
最后更新：2026-07-23
状态：M0 初始化完成；M1 SDL 实体输入、ViGEm X360 输出与本机震动桥接完成，待掌机实测桥接
许可证：GPL-3.0-only
...
当前公开状态结构均以 `struct_size` 开头：`rc_session_snapshot_v1` 为 56 字节，`rc_sdl_runtime_info_v1` 为 336 字节，`rc_input_device_info_v1` 为 712 字节，`rc_input_capture_snapshot_v1` 为 64 字节，`rc_vigem_runtime_info_v1` 为 272 字节，`rc_local_bridge_snapshot_v1` 为 56 字节；`rc_gamepad_state_v1` 与 wire 明文同为 16 字节。opaque handle 不跨 DLL 暴露 C++、STL 或线程对象。Dart `LoopbackSession`、`SdlInputCapture` 和 `LocalControllerBridge` 都是显式 `close()` 资源；Flutter 只以 10 Hz 读取诊断快照，250 Hz 采样、ViGEm 提交和震动执行不经 Dart 往返。
...
- Core 10 项和 Flutter 应用 7 项测试已通过；ViGEmBus probe 与“target 创建成功后输入打开失败”的清理路径已在开发机验证。
- Windows x64 Release 已构建。发布目录包含 `remote_controller.exe`、`remote_controller_core.dll`、`SDL3.dll` 和 Flutter 运行库；`remote_controller_core.dll` 的动态依赖只有系统 `SETUPAPI.dll`/`KERNEL32.dll`，没有单独的 `ViGEmClient.dll`。开发机 `ViGEmBus` 服务为 Running；掌机实际桥接和震动仍待下一轮实测。

紧接任务：

1. 在 ROG Ally X 上启动“桥接到虚拟 X360”，用 Windows 游戏控制器面板或测试游戏验证按钮/轴/扳机、target 销毁和双马达震动回传；当前不要在要求独占输入的游戏里长时间运行。
2. 补充不同 Armoury Crate 控制模式下的 SDL 名称、GUID、路径和 Windows 设备拓扑矩阵，为 HidHide 精确目标选择提供依据。
3. 把本机桥接拆分为未来 Client/Server 两侧可复用的 session 组合：Client SDL → transport，Server transport → ViGEm，并复用现有 sequence/watchdog 安全底座。
4. 实现 M2 最小局域网通道和能力握手，保持控制/震动可靠通道与低延迟完整状态通道分离。
5. 设计并实现最小特权 HidHide broker；在实机独占验证前保持默认不自动修改驱动配置。
6. 向 `native_toolchain_c` 上游报告中文 `vswhere -utf8` 解码和 C++ `/TP` 链接问题，升级后移除兼容代码。
